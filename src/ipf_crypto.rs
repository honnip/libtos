...
impl ZipCrypto {
    fn new() -> Self {
        ...
        for password in PASSWORD {
            Self::update_key(&mut ret, password)
        }

        ret
    }

    fn crc32(crc: Wrapping<u32>, char: u8) -> Wrapping<u32> {
        (crc >> 8) ^ Wrapping(CRC_TABLE[((crc & Wrapping(0xFF)).0 as u8 ^ char) as usize])
    }

    fn update_key(&mut self, password: u8) {
        self.key_0 = Self::crc32(self.key_0, password);
        self.key_1 =
            (self.key_1 + (self.key_0 & Wrapping(0xFF))) * Wrapping(0x08088405) + Wrapping(1);
        self.key_2 = Self::crc32(self.key_2, (self.key_1 >> 24).0 as u8);
    }

    fn stream_byte(&mut self) -> u8 {
        let temp = Wrapping(self.key_2.0 as u16) | Wrapping(3);
